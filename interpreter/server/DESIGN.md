...
2. **get_session()**：直接从文件系统获取指定的会话信息
3. **list_sessions()**：列出所有会话，按最后活跃时间降序排序
4. **acquire_session_lock()/release_session_lock()**：获取/释放会话锁
5. **mark_instance_status()**：封装对实例状态的标记

#### 基于文件系统的持久化存储

1. **存储路径**：
   - 使用`platformdirs`获取系统配置目录
   - 默认存储在`~/Library/Application Support/open-interpreter/conversations`
...
4. **会话排序机制**：
   - 会话列表按照`last_active`字段降序排序
   - 确保最新会话显示在前
   - 如无有效时间戳，则使用文件修改时间作为替代
...
2. **process_streaming_chat()**：
   - 处理流式聊天请求
   - 逐步生成响应并通过迭代器返回
   - 支持文本生成、代码生成和执行过程的流式展示
   - 自动处理流式响应的格式转换（原生/OpenAI格式）
...
4. **API支持**：
   - 提供原生API (`/v1/chat`)
   - 提供OpenAI兼容API (`/v1/chat/completions`)
   - 统一的消息处理逻辑，根据请求类型输出不同格式
